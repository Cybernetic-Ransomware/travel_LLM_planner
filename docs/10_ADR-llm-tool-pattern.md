...
- Read-only tools (implemented: `list_saved_trips`, `get_trip_details`) follow the same factory pattern but omit the `allowed_place_ids` scope guard — they perform no writes, and saved trips are not session-scoped resources.
...
- **Resume re-appended the client's last message** ✅ resolved (bug): on confirm/cancel the router passed `payload.messages[-1].content` as `user_message`, and the manager appended it to the thread before resuming. The client history is already checkpointed, so this inserted a duplicate `HumanMessage` between the interrupted `AIMessage(tool_calls)` and the future `ToolMessage` — invalidating the history for the LLM provider (OpenAI 400) and recomputing the conditional edge to `END`, so the confirmed tool never executed. The router now always passes `user_message=None`; the parameter remains on `astream_resume` for future clients that submit new text alongside a confirmation.
- **Dangling `tool_calls` from abandoned proposals** ✅ resolved: a proposal the user never answered (typing a new message instead, or the frontend-only cancel) left an `AIMessage` with unanswered `tool_calls` in the checkpointed thread, and the LLM provider rejected every subsequent turn with a 400 error. `OrchestratorManager.acancel_pending_tools(thread_id)` scans the thread history and replaces every `AIMessage` whose `tool_calls` have no matching `ToolMessage` responses (using the same `add_messages` ID-replacement mechanism as rejection). It is invoked by the router on every normal turn that carries an existing `session_id`, and by `astream_resume(confirmed=False)`. The frontend cancel button now also notifies the backend via `resume_confirmed: false` instead of only clearing local UI state.
