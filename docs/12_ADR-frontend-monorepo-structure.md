...
- **uv workspace with `apps/api/` and `apps/frontend/`** — the correct approach if two independent Python services need to share dependencies. The project currently has one Python service. Introducing a workspace to satisfy a naming preference introduces `[tool.uv.workspace]` configuration, nested `pyproject.toml` files, and changes to how `uv sync` resolves the environment. Premature and reversible if a second service emerges.
...
- `apps/api/` exists as an empty placeholder. It serves as a future home if the Python service is ever extracted into a dedicated sub-package (uv workspace migration). It is not used and contains no files.
...
### Future Potential
- If a second Python service (e.g. a background worker) is introduced, migrating to a uv workspace is a well-defined operation: create `apps/api/pyproject.toml`, move `src/`, update the workspace root. ADR-12 would be superseded at that point.
...
- **Asymmetry `src/` vs `apps/frontend/`**: may confuse contributors expecting full symmetry. Mitigated by this ADR and the `apps/api/` placeholder signalling the intended long-term shape.
