...
- **BSON serialization of checkpoint payloads** ✅ resolved: the initial implementation stored the `Checkpoint` and `CheckpointMetadata` dicts directly in the document. BSON cannot encode LangChain message objects (`HumanMessage`, `AIMessage`), so `aput` crashed with `bson.errors.InvalidDocument` on the first checkpoint containing `tool_calls` — killing the SSE stream mid-turn. The saver now serializes both payloads via the inherited `self.serde` (`JsonPlusSerializer.dumps_typed`), storing them as BSON Binary alongside `checkpoint_type` / `metadata_type` tags, and `aget_tuple` restores them with `loads_typed`. Documents written in the old format are unreadable by the new code, but they expire via the TTL index and no migration was required at this stage of the project.
