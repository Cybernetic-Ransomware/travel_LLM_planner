...
def list_places(*, skipped: bool | None = None, list_name: str | None = None) -> list[dict]:
    ...
    r = httpx.get(f"{_BASE}/places", params=params, headers=_auth_headers())
    _raise_for_status(r)
    return r.json()
...
def patch_place(place_id: str, payload: dict) -> dict:
    ...
    r = httpx.patch(f"{_BASE}/places/{place_id}", json=payload, headers=_auth_headers())
    _raise_for_status(r)
    return r.json()
...
def delete_place(place_id: str) -> None:
    ...
    r = httpx.delete(f"{_BASE}/places/{place_id}", headers=_auth_headers())
    _raise_for_status(r)


def import_list(list_url: str) -> dict:
    """Scrape a public Google Maps list and upsert places. May take up to 2 minutes."""
    r = httpx.post(f"{_BASE}/import", json={"list_url": list_url}, timeout=120.0, headers=_auth_headers())
    _raise_for_status(r)
    return r.json()
...
def enrich_places(limit: int = 20) -> dict:
    ...
    r = httpx.post(f"{_BASE}/enrich", json={"limit": limit}, timeout=120.0, headers=_auth_headers())
    _raise_for_status(r)
    return r.json()
...
def optimize_route(payload: dict) -> dict:
    ...
    r = httpx.post(f"{_OPTIMIZER_BASE}/route", json=payload, timeout=30.0, headers=_auth_headers())
    _raise_for_status(r)
    return r.json()
...
def optimize_trip(payload: dict) -> dict:
    ...
    r = httpx.post(f"{_OPTIMIZER_BASE}/trip", json=payload, timeout=60.0, headers=_auth_headers())
    _raise_for_status(r)
    return r.json()
