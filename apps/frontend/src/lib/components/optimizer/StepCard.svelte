...
<div class="flex gap-3 rounded-lg border border-zinc-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-900">
	<div
	...
	<div class="min-w-0 flex-1">
		...
		<div class="mt-0.5 flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-zinc-500 dark:text-zinc-400">
			...
			{#if step.travel_from_previous_s > 0}
				<span class="text-zinc-400 dark:text-zinc-500">+{formatDuration(step.travel_from_previous_s)} {m.step_travel_unit()}</span>
			{/if}
			{#if step.wait_min > 0}
				<span class="text-zinc-400 dark:text-zinc-500">{step.wait_min} min {m.step_wait_unit()}</span>
			{/if}
		</div>
	</div>
</div>
