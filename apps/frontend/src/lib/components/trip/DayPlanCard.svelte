<script lang="ts">
	...
	const totalVisitMin = $derived(
		dayPlan.steps.reduce((s, step) => s + step.visit_duration_min, 0)
	);
...
<div class="flex flex-col gap-2">
	<div class="flex items-center justify-between">
		<h3 class="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
			{m.day_label()} {dayPlan.day_index + 1}
	...
	{:else}
		<p class="rounded-md border border-dashed border-zinc-300 py-4 text-center text-xs text-zinc-400 dark:border-zinc-700">
			{m.results_no_schedule()}
		</p>
	{/if}

	{#if dayPlan.skipped.length > 0}
		<div class="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 dark:border-amber-800 dark:bg-amber-950">
			<p class="text-xs font-semibold text-amber-700 dark:text-amber-300">
				{dayPlan.skipped.length} {m.results_skipped_label()}
			</p>
